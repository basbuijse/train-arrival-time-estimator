...
import utils
from models.base import Attr
from models.base import SpatioTemporal
import numpy as np
...
class EntireEstimator(nn.Module):
    ...
    def forward(self, attr_t, sptm_t):
        ...
        hidden = F.leaky_relu(self.input2hid.cpu()(inputs))

        for i in range(len(self.residuals)):
            residual = F.leaky_relu(self.residuals[i].cpu()(hidden))
            #residual = F.leaky_relu(self.residuals[i](hidden))
            hidden = hidden + residual

        out = self.hid2out.cpu()(hidden)
...
class LocalEstimator(nn.Module):
    ...
    def forward(self, sptm_s):
        hidden = F.leaky_relu(self.input2hid.cpu()(sptm_s))

        hidden = F.leaky_relu(self.hid2hid.cpu()(hidden))
        
        out = self.hid2out.cpu()(hidden)

        return out

    def eval_on_batch(self, pred, lens, label, mean, std, config):
        label, batch_size, _, _ = nn.utils.rnn.pack_padded_sequence(label, lens, batch_first = True)
        label = label.view(-1, 1)
        ...
        label = label.view(-1)
        label = nn.utils.rnn.PackedSequence(label, batch_size)
        label_unpacked, label_lens_unpacked = nn.utils.rnn.pad_packed_sequence(label, batch_first=True)
        
        pred = pred.view(-1)
        pred = nn.utils.rnn.PackedSequence(pred, batch_size)
        pred_unpacked, pred_lens_unpacked = nn.utils.rnn.pad_packed_sequence(pred, batch_first=True)
        
        label_unpadded = []
        for i in range(len(label_unpacked)):
            length = label_lens_unpacked[i].item()
            label_unpadded += [label_unpacked[i][:length]]
        
        pred_unpadded = []
        for i in range(len(pred_unpacked)):
            length = pred_lens_unpacked[i].item()
            pred_unpadded += [pred_unpacked[i][:length]]

        label = torch.cat(label_unpadded).flatten()
        label = label.view(-1, 1)
        label = torch.round(label * config['time_gap_std'])
        
        pred = torch.cat(pred_unpadded).flatten()
        pred = pred.view(-1, 1)
        pred = pred * config['time_gap_std']
...
class Net(nn.Module):
    ...
    def eval_on_batch(self, attr, traj, config):
        ...
        pred_dict_local, local_loss = self.local_estimate.eval_on_batch(local_out, local_length, local_label, mean, std, config)
        if self.training:
            return pred_dict, pred_dict_local, local_length, (1 - self.alpha) * entire_loss + self.alpha * local_loss
        else:
            return pred_dict, pred_dict_local, local_length, (1 - self.alpha) * entire_loss + self.alpha * local_loss
